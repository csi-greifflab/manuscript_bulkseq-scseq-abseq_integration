...
ggplot(count_sample_all) +
  geom_col(aes(x = Sample, y = Count), fill = "grey", color = "black") +
...
```{r contigs_track, fig.height=8, fig.width=12}
...
```
\newpage
```{r contigs_track_prop, fig.height=8, fig.width=12}
ggplot(count_processing) +
...
map2(.x = sample_all_clonotypes_heavy, .y = names(sample_all_clonotypes_heavy), function (x,y) write_tsv(x, file = paste0("./4_Ig_all_clonotypes_heavy/",y,"_heavy.txt")))
map2(.x = sample_all_clonotypes_light, .y = names(sample_all_clonotypes_light), function (x,y) write_tsv(x, file = paste0("./4_Ig_all_clonotypes_light/",y,"_light.txt")))
```

# Split the dataset into distinct isotypes
```{r}
...
```{r}
data_d1_scseq <- repLoad("./5_Ig_all_clonotypes_isotypes/")

# Load the mixcr files using read_tsv
files_mixcr <- list.files("./5_Ig_all_clonotypes_isotypes/", full.names = T)
...
for (i in 1:length(data_d1_scseq$data)){
  data_d1_scseq$data[[i]]$Clones <- Ig_all_mixcr[[i]]$cellCount
  data_d1_scseq$data[[i]]$V.name <- Ig_all_mixcr[[i]]$bestVGene
  data_d1_scseq$data[[i]]$D.name <- Ig_all_mixcr[[i]]$bestDGene
  data_d1_scseq$data[[i]]$J.name <- Ig_all_mixcr[[i]]$bestJGene
  data_d1_scseq$data[[i]]$C.name <- Ig_all_mixcr[[i]]$bestCGene
  data_d1_scseq$data[[i]]$Sequence.aa <- Ig_all_mixcr[[i]]$aaSeqVDJRegion
  data_d1_scseq$data[[i]]$Sequence.aa.imputed <- Ig_all_mixcr[[i]]$aaSeqImputedVDJRegion
  data_d1_scseq$data[[i]]$UMI.count <- Ig_all_mixcr[[i]]$uniqueUMICount
  data_d1_scseq$data[[i]]$Barcode <- Ig_all_mixcr[[i]]$tagValueCELL
}

# Sort the df by descending number of clones
data_d1_scseq$data <- map(data_d1_scseq$data, function(x) arrange(x, desc(Clones)))

# Rename the isotype to be more concise, recalculate the clonal proportion, get the full-length aa sequence
for (i in 1:length(data_d1_scseq$data)){
  data_d1_scseq$data[[i]]$Proportion <- data_d1_scseq$data[[i]]$Clones/sum(data_d1_scseq$data[[i]]$Clones)}

# Save the processed immunarch object as RDS
saveRDS(data_d1_scseq, "./data_d1_scseq.RDS")
```
